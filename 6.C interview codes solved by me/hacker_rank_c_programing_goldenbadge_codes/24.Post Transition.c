...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {
  ...
  for (int i = 0; i < source->offices[source_office_index].packages_count; i++)
    if (source->offices[source_office_index].packages[i].weight >=target->offices[target_office_index].min_weight &&source->offices[source_office_index].packages[i].weight <=target->offices[target_office_index].max_weight)
      new_package_counter++;
  package *newPackages = malloc(sizeof(package) * (new_package_counter + target->offices[target_office_index].packages_count));

  package *oldPackages = malloc(sizeof(package) * (source->offices[source_office_index].packages_count -new_package_counter));
  ...
  new_package_counter = target->offices[target_office_index].packages_count;
  int old_package_counter = 0;
  for (int i = 0; i < source->offices[source_office_index].packages_count; i++)
    if (source->offices[source_office_index].packages[i].weight >= target->offices[target_office_index].min_weight &&source->offices            [source_office_index].packages[i].weight <=target->offices[target_office_index].max_weight) {
      newPackages[new_package_counter] =source->offices[source_office_index].packages[i];
      new_package_counter++;
    }
    else {
      oldPackages[old_package_counter] =source->offices[source_office_index].packages[i];
      old_package_counter++;
    }
  target->offices[target_office_index].packages_count = new_package_counter;
  free(target->offices[target_office_index].packages);
  target->offices[target_office_index].packages = newPackages;
  source->offices[source_office_index].packages_count = old_package_counter;
