 ...
 * PWM0.c
 *
 * Created: 8/12/2024 8:28:40 PM
 *  Author: ismail
 */ 
#include "PWM0.h"

Timer0callback PWM_CALLBACK_OVER_FLOW;
Timer0callback PWM_CALLBACK_COMPARE;
...
void PWM0_Init(PWM_t* configuartion){
		...
		if ((PWM_Global_configuartion->Mode == Fast_PWM) || (PWM_Global_configuartion->Mode == Phase_Correct_PWM)){
		...
		if ((PWM_Global_configuartion->CLK >= TMR0NO_CLK_SRC) && (PWM_Global_configuartion->CLK <= TMR0EXT_CLK_RISING)){
		...
		if ((PWM_Global_configuartion->Inverting == PWM_INVERTING) || (PWM_Global_configuartion->Inverting == PWM_Non_INVERTING)){
			//TCCR0 |= PWM_Global_configuartion->Invering;
			if(PWM_Global_configuartion->Inverting == PWM_INVERTING){
				SET_BIT(TCCR0,COM00);
				SET_BIT(TCCR0,COM01);
			}
			else{
